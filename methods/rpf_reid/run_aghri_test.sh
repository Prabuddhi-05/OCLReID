...
RESULTS_ROOT="${OCLREID_RESULTS_ROOT:-${ROOT}/results/reproduced_runs/rpf_reid}"
DRY_RUN=0
MAX_RUNS=()
OVERWRITE=()
SAVE_VIS=()
...
CMD=(conda run -n "${ENV_NAME}" python "${ROOT}/scripts/run_aghri_experiments.py" --splits test --methods rpf-ReID --dataset-root "${AGHRI_DATASET_ROOT}" --video-root "${AGHRI_VIDEO_ROOT}" --results-root "${RESULTS_ROOT}" --reid-checkpoint "${REID_CHECKPOINT}" "${MAX_RUNS[@]}" "${TARGET_CLASSES[@]}" "${OVERWRITE[@]}" "${SAVE_VIS[@]}" "${SHOW_LIVE[@]}")

...
class PartRPF(BaseRobotPersonFollower):
    ...
    def has_initialization_bbox(self, gt_bbox):
        if gt_bbox is None:
            return False
        try:
            bbox = np.asarray(gt_bbox, dtype=float)
        except (TypeError, ValueError):
            return False
        return bbox.ndim == 1 and bbox.size >= 4 and np.all(np.isfinite(bbox[:4]))
    ...
    def simple_test(self,
        ...
        if len(track_bboxes)!=0 and self.target_id is None and self.has_initialization_bbox(gt_bbox):

...
class PartTracklet:
    def __init__(self, img, img_metas, params, observation, rescale=False, img_scale=(256, 128)):
        """
        ...
        if len(observation) > 4:
            ...
            if self.params.use_joint:
                ...
                self.keypoint_confidence_summary = self.get_keypoint_confidence_summary(self.kpts)
        ...
        self.visible_part_count = int(torch.sum(self.visibility_indicator).item())
        self.orientation_history = []
        self.orientation_flip_count = 0
        self.orientation_stability_score = 1.0
    ...
    def get_keypoint_confidence_summary(self, kpts):
        parts = {
            "head": [0],
            "torso": [1, 2, 13],
            "legs": [7, 8, 9, 10],
        ...
        kpts = np.array(kpts)
        confidences = kpts[:, 2].astype(float) if kpts.size else np.array([], dtype=float)
        per_part = {}
        for name, indexes in parts.items():
            vals = [float(confidences[index]) for index in indexes if index < len(confidences)]
            per_part[name] = float(np.mean(vals)) if vals else None
        return {
            "mean": float(np.mean(confidences)) if len(confidences) else None,
            "minimum": float(np.min(confidences)) if len(confidences) else None,
            "per_part": per_part,
            "head": per_part["head"],
            "torso": per_part["torso"],
            "legs": per_part["legs"],
            "feet": per_part["feet"],
        }
    ...
    def crop_imgs(self, img, img_metas, bbox, rescale=False):
        ...
        mean = torch.tensor([0.485, 0.456, 0.406], device=crop_img.device).view(1, 3, 1, 1)
        std = torch.tensor([0.229, 0.224, 0.225], device=crop_img.device).view(1, 3, 1, 1)
        # 归一化操作

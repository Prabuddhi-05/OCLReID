...
from .reliability_fusion import (
    VALID_MODES,
    active_counts,
    aggregate_dual_orientation,
    blend_global_fallback,
    bounded_score,
    global_slot,
    pose_reliability,
)
...
class PartOCLWeightedClassifier(BaseClassifier):
    ...
    def predict(self, tracklets: dict, state="tracking"):
        ...
        for idx in sorted(tracklets.keys()):
            ...
            target_score = self._apply_reliability_mode(idx, tracklets[idx], start_idx, seg_idx, vis_indicator, feature, part_scores, target_score)
    ...
    def _mode(self):
        mode = getattr(self.params, "reliability_mode", "baseline")
        return mode if mode in VALID_MODES else "baseline"

    def _history(self, idx, binary_ori):
        length = int(getattr(self.params, "orientation_history_length", 5))
        history = self.orientation_histories.setdefault(idx, [])
        history.append(int(binary_ori))
        if length > 0 and len(history) > length:
            del history[:-length]
        flips = sum(1 for left, right in zip(history, history[1:]) if left != right)
        threshold = max(1, int(getattr(self.params, "orientation_flip_threshold", 2)))
        stability = max(0.0, 1.0 - flips / float(threshold))
        return list(history), flips, float(stability)

    def _score_bank(self, start_idx, seg_idx, vis_indicator, feature):
        scores, score = self._tracking_predict(start_idx, seg_idx, vis_indicator, feature)
        return scores, score if scores else None

    def _apply_reliability_mode(self, idx, tracklet, start_idx, seg_idx, vis_indicator, feature, part_scores, target_score):
        mode = self._mode()
        hard_score = float(target_score)
        history, flips, orientation_stability = self._history(idx, tracklet.binary_ori)
        confidence_summary = getattr(tracklet, "keypoint_confidence_summary", {})
        visible_part_count = getattr(tracklet, "visible_part_count", None)
        reliability = pose_reliability(
            confidence_summary.get("mean"),
            visible_part_count,
            float(getattr(self.params, "pose_confidence_threshold", 0.0)),
            int(getattr(self.params, "minimum_visible_parts", 0)),
            float(getattr(self.params, "minimum_reliability", 0.0)),
        )
        current_global_slot = global_slot(start_idx, seg_idx)
        current_global_score = part_scores.get(current_global_slot)
        final_score = hard_score
        selected_aggregation = "baseline"
        front_scores = {}
        back_scores = {}
        front_score = None
        back_score = None

        if mode in {"dual_orientation", "combined"} and self.params.use_ori:
            front_scores, front_score = self._score_bank(0, self.part_nums // 2, vis_indicator, feature)
            back_scores, back_score = self._score_bank(self.part_nums // 2, self.part_nums // 2, vis_indicator, feature)
            threshold = int(getattr(self.params, "orientation_flip_threshold", 2))
            if flips >= threshold:
                final_score, selected_aggregation = aggregate_dual_orientation(
                    hard_score,
                    front_score,
                    back_score,
                    orientation_stability,
                    getattr(self.params, "dual_orientation_aggregation", "min"),
                )
                merged = dict(front_scores)
                merged.update(back_scores)
                part_scores.clear()
                part_scores.update(merged)
            else:
                selected_aggregation = "hard_orientation_stable"

        if mode in {"global_fallback", "combined"}:
            global_score = current_global_score
            if mode == "combined" and selected_aggregation.startswith("dual_orientation"):
                valid_global = []
                front_global = front_scores.get(self.part_nums // 2 - 1)
                back_global = back_scores.get(self.part_nums - 1)
                for value in [front_global, back_global]:
                    if value is not None:
                        valid_global.append(value)
                if valid_global:
                    global_score = sum(valid_global) / len(valid_global)
            final_score, blend_alpha = blend_global_fallback(
                final_score,
                global_score,
                reliability,
                float(getattr(self.params, "global_fallback_weight", 0.0)),
            )
        else:
            blend_alpha = 0.0
            global_score = current_global_score

        final_score = bounded_score(final_score, [hard_score, global_score, front_score, back_score])
        counts = active_counts(part_scores, self.part_nums // 2, self.part_nums)
        tracklet.orientation_history = history
        tracklet.orientation_flip_count = flips
        tracklet.orientation_stability_score = orientation_stability
        tracklet.reliability_scores = {
            "pose_reliability": reliability,
            "orientation_reliability": orientation_stability,
        }
        tracklet.score_components = {
            "reliability_mode": mode,
            "current_part_average_score": hard_score,
            "current_global_score": global_score,
            "front_bank_score": front_score,
            "back_bank_score": back_score,
            "prototype_final_score": final_score,
            "global_fallback_alpha": blend_alpha,
            "selected_aggregation": selected_aggregation,
            **counts,
            "global_classifier_available": global_score is not None,
        }
        return final_score
